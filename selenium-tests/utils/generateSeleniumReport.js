/**
 * generateSeleniumReport.js — FleetSync Selenium Web E2E Test Report
 ...
 *
 * Mirrors the Appium report format from appium-tests/utils/generateExcelReport.js
 */

const fs   = require('fs');
const path = require('path');
const ExcelJS = require('exceljs');
const { generateAllMetadata, MODULES } = require('./seleniumTestMetadata');
...
async function main() {
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // Sheet 1: E2E Functional Test Cases (matches the Appium report format)
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // Column widths
  s1.getColumn(1).width = 12;  // Test ID
  s1.getColumn(2).width = 28;  // Module
  ...
  titleCell.value     = '🌐  FleetSync — Selenium Website E2E Test Report';
  ...
  subCell.value     = `Generated: ${new Date().toUTCString()} | Environment: Headless Ubuntu 22.04 | CI: GitHub Actions`;
  ...
  if (process.env.GITHUB_STEP_SUMMARY) {
    const summary = `## 🌐 Selenium Website E2E — Results\n`
      + `| Metric | Value |\n|---|---|\n`
      + `| Total Tests | ${total} |\n`
      + `| ✅ Passed | ${passed} |\n`
      + `| ❌ Failed | ${failed} |\n`
